...
def notifyen():
    try:
        noti = requests.get("https://raw.githubusercontent.com").text.upper().strip()
        if len(noti) > 10:
...
def pretty_print(cc,target,success,failed):
    ...
    mesgdcrt.SuccessMessage("TBomb was created by Unknown Hacker")
...
def selectnode(mode="sms"):
    ...
    try:
        ...
        max_limit={"sms":5000,"call":5000,"mail":2000}
        cc,target="",""
        if mode in ["sms","call"]:
...
__CONTRIBUTORS__ = ['Unknown Hacker','t0xic0der','scpketer','Stefan']
...
parser = argparse.ArgumentParser(description=description,epilog='Coded by Unknown Hacker !!!')
